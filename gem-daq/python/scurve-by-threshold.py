...
glib.set("vfat2_0_channel14", (1 << 6) + trims[13])
...
f = open('scurve-threshold.txt', 'w')
f.write("threshold\tvcal\tpercentage\n")
...
for threshold in range(THRESHOLD_MIN, THRESHOLD_MAX):
    ...
    for d in data:
        ...
        f.write(str(threshold) + "\t" + str((d & 0xff000000) >> 24) + "\t" + str((d & 0xffffff) / N_EVENTS) + "\n")
