...
u08 SDR7[] = {
	// sensor record header
	0x07,           // record number, LSB  - filled by SDR_Init()
	0x00,           // record number, MSB  - filled by SDR_Init()
	0x51,           // IPMI protocol version
	0x01,           // record type: full sensor
	0x36,           // record length: remaining bytes -> SDR_Init
	// record key bytes
	0x00,           // i2c address, -> SDR_Init
	0x00,           // sensor owner LUN
	...
	0x00,           // sensor units 1 : Unsigned		// changed by VB for the MCP sensor...
	0x01,           // sensor units 2 : Temperature
	0x00,           // sensor units 3 : Modifier
	0x00,           // Linearization
	0x05,           // M								// M = 2 changed by VB for the MCP sensor...
	0x01,           // M - Tolerance
	0x00,           // B
	0x0a,           // B - Accuracy						// Accuracy set to 10%. Changed by VB for the MCP sensor...
	0x00,           // Sensor direction					// changed by VB for the MCP sensor...
	0xf0,           // R-Exp , B-Exp					// Rexp = -1 changed by VB for the MCP sensor...
	0x07,           // Analogue characteristics flags
	50,             // Nominal reading					// changed by VB for the MCP sensor...
	120,             // Normal maximum					// changed by VB for the MCP sensor...
	20,             // Normal minimum					// changed by VB for the MCP sensor...
	127,            // Sensor Maximum reading
	0,           // Sensor Minimum reading				// changed by VB for the MCP sensor...
	85,             // Upper non-recoverable Threshold
	75,             // Upper critical Threshold
	65,             // Upper non critical Threshold
	...
	'M','C','P','9','8','0','8','-','I','C','4' // sensor string
};
...
u08 SDR8[] = {
	// sensor record header
	0x08,           // record number, LSB  - filled by SDR_Init()
	0x00,           // record number, MSB  - filled by SDR_Init()
	0x51,           // IPMI protocol version
	0x01,           // record type: full sensor
	0x37,           // record length: remaining bytes -> SDR_Init
	...
	0x00,           // sensor units 3 : Modifier
	0x00,           // Linearization
	0x05,           // M								// M = 5 changed by VB for the MCP sensor...
	0x01,           // M - Tolerance
	0x00,           // B
	0x0a,           // B - Accuracy						// Accuracy set to 10%. Changed by VB for the MCP sensor...
	0x00,           // Sensor direction					// changed by VB for the MCP sensor...
	...
	0x00,           // OEM reserved
	0xcc,           // 8 bit ASCII, number of bytes
	'M','C','P','9','8','0','8','-','I','C','1','2' // sensor string
};
...
u08 SDR10[] = {
	...
	0x00,           // i2c address, -> SDR_Init
	0x00,           // sensor owner LUN
	TEMPERATURE_SENSOR5,    // sensor number
	// record body bytes
	0xc1,           // entity id: AMC Module
	0x00,           // entity instance -> SDR_Init
	0x03,           // init: event generation + scanning enabled
	...
	0x00,           // OEM reserved
	0xcc,           // 8 bit ASCII, number of bytes
	'M','C','P','9','8','0','8','-','I','C','1','1' // sensor string
};
...
u08 SDR11[] = {
	...
	0x00,           // i2c address, -> SDR_Init
	0x00,           // sensor owner LUN
	TEMPERATURE_SENSOR6,    // sensor number
	// record body bytes
	0xc1,           // entity id: AMC Module
	0x00,           // entity instance -> SDR_Init
	0x03,           // init: event generation + scanning enabled
	...
	0x00,           // OEM reserved
	0xcc,           // 8 bit ASCII, number of bytes
	'M','C','P','9','8','0','8','-','I','C','3' // sensor string
};
...
u08 SDR12[] = {
	...
	0x00,           // i2c address, -> SDR_Init
	0x00,           // sensor owner LUN
	TEMPERATURE_SENSOR7,    // sensor number
	// record body bytes
	0xc1,           // entity id: AMC Module
	0x00,           // entity instance -> SDR_Init
	0x03,           // init: event generation + scanning enabled
	...
	0x00,           // OEM reserved
	0xcc,           // 8 bit ASCII, number of bytes
	'M','C','P','9','8','0','8','-','I','C','1','0' // sensor string
};
...
u08 SDR13[] = {
	...
	0x00,           // i2c address, -> SDR_Init
	0x00,           // sensor owner LUN
	TEMPERATURE_SENSOR8,    // sensor number
	// record body bytes
	0xc1,           // entity id: AMC Module
	0x00,           // entity instance -> SDR_Init
	0x03,           // init: event generation + scanning enabled
	...
	0x00,           // OEM reserved
	0xcc,           // 8 bit ASCII, number of bytes
	'M','C','P','9','8','0','8','-','I','C','2' // sensor string
};
...
u08 *sdrPtr[NUM_SDR_AMC + NUM_SDR_RTM] = {SDR0, SDR1, SDR2, SDR3, SDR4, SDR5, SDR6, SDR7, SDR8, SDR9, SDR10, SDR11, SDR12, SDR13, SDR14, SDR15, SDR16, SDR17, SDR18};
u08 sdrLen[NUM_SDR_AMC + NUM_SDR_RTM] =  {sizeof(SDR0), sizeof(SDR1), sizeof(SDR2), sizeof(SDR3), sizeof(SDR4), sizeof(SDR5), sizeof(SDR6), sizeof(SDR7), sizeof(SDR8),
        sizeof(SDR9), sizeof(SDR10), sizeof(SDR11), sizeof(SDR12), sizeof(SDR13), sizeof(SDR14), sizeof(SDR15), sizeof(SDR16), sizeof(SDR17), sizeof(SDR18)};
...
//********************/
{
	MCP_temp_init(TEMPERATURE_SENSOR3, 0);		// init of the MCP9808 IC.
	MCP_temp_init(TEMPERATURE_SENSOR4, 0);		// init of the MCP9808 IC.
	MCP_temp_init(TEMPERATURE_SENSOR5, 0);
	MCP_temp_init(TEMPERATURE_SENSOR6, 0);
	MCP_temp_init(TEMPERATURE_SENSOR7, 0);
	MCP_temp_init(TEMPERATURE_SENSOR8, 0);
}

/***************************/
void MCP_temp_init(u08 sensorID, u08 ByteToSend)   //Called from sensor_init_user() in user_code.c
/***************************/
{
	u08 saddr;     //temperature sensor address
	if (sensorID > TEMPERATURE_SENSOR2)			// no init required for the other temp sensors.
	{
		saddr = resolve_sensor_add(sensorID);
		CRITICAL_SECTION_START;
		i2cswSend(saddr, 8, 1, 1, &ByteToSend);		// Init for the MCP9808 IC. Goal is to set the resolution register to +/- 0.5°C.
		CRITICAL_SECTION_END;
	}
}
...
/***************************/
{
	...
	sens[TEMPERATURE_SENSOR3].value  = MCP_temp_sensor_read(TEMPERATURE_SENSOR3);		// VB: this has been replaced by an MCP9808 sensor.
	if(run_type == INCRATE)
	check_temp_event(TEMPERATURE_SENSOR3);

	sens[TEMPERATURE_SENSOR4].value  = MCP_temp_sensor_read(TEMPERATURE_SENSOR4);		// VB: this has been replaced by an MCP9808 sensor.
	if(run_type == INCRATE)
	check_temp_event(TEMPERATURE_SENSOR4);
	
	sens[TEMPERATURE_SENSOR5].value  = MCP_temp_sensor_read(TEMPERATURE_SENSOR5);		// MDC: this has been replaced by an MCP9808 sensor.
	if(run_type == INCRATE)
	check_temp_event(TEMPERATURE_SENSOR5);
	
	sens[TEMPERATURE_SENSOR6].value  = MCP_temp_sensor_read(TEMPERATURE_SENSOR6);		// MDC: this has been replaced by an MCP9808 sensor.
	if(run_type == INCRATE)
	check_temp_event(TEMPERATURE_SENSOR6);
	
	sens[TEMPERATURE_SENSOR7].value  = MCP_temp_sensor_read(TEMPERATURE_SENSOR7);		// MDC: this has been replaced by an MCP9808 sensor.
	if(run_type == INCRATE)
	check_temp_event(TEMPERATURE_SENSOR7);
	
	sens[TEMPERATURE_SENSOR8].value  = MCP_temp_sensor_read(TEMPERATURE_SENSOR8);		// MDC: this has been replaced by an MCP9808 sensor.
	if(run_type == INCRATE)
	check_temp_event(TEMPERATURE_SENSOR8);
	...
	sens[VOLT_12].value = PP_sensor_read();
	sens[VOLT_3_3].value = MP_sensor_read();
}


/***************************/
u08 MCP_temp_sensor_read(u08 sensor)   //Called from user_code.c Conversion for MCP9808 temp sensor IC
/***************************/
{
	u08 saddr, bytes[2];
	u16 stemp;

	saddr = resolve_sensor_add(sensor);

	CRITICAL_SECTION_START;
	i2cswReceive(saddr, 0x05, 1, 2, &bytes[0]);	// MDC: No neet to write, just read the Ta register. Read the MSB and LSB.
	CRITICAL_SECTION_END;

	bytes[0] = bytes[0] & 0x1F; 		//Clear the unused flag bits
	if ((bytes[0] & 0x10) == 0x10)		//TA < 0°C
	stemp = 0;
	else 								//TA > 0°C
	{
		//stemp = (bytes[0] << 8) + bytes[1];
		//stemp = (bytes[0] << 4) + (bytes[1] >> 4); //MDC
		stemp = (bytes[0] << 5) + (bytes[1] >> 3);
		//stemp = 0xff & (stemp >> 3);	// ignore the 3 LSB as the resolution is set to 0.5°C.
	}
	stemp = (u08)(stemp);
	return stemp;
}
...
/***************************/
u08 resolve_sensor_add(u08 sensor)	//Called from user_code.c Resolves temp sensor I2C adresses.
/***************************/
{
	u08 saddr;     //temperature sensor address
	// VB: I guess this could be replaced by a case structure.
	...
	saddr = 0x19;
	if  (sensor == TEMPERATURE_SENSOR5)		
	saddr = 0x1A;
	if (sensor == TEMPERATURE_SENSOR6)
	saddr = 0x1B;
	if (sensor == TEMPERATURE_SENSOR7)
	saddr = 0x1C;
	if (sensor == TEMPERATURE_SENSOR8)
	saddr = 0x1D;
...
//*******************************/
u08 PP_sensor_read()   //Called from user_code.c
//*******************************/
{
	u08 saddr, scmd, bytes[2];
	//static float Result;
	//static u16 sResult = 0;
	u08 Result = 0;
	//float Result = 0;
	
	scmd  = 0x80;
	saddr = 0x22; //Board ADC I2C address
    ...
    Result = (bytes[0] << 4) + (bytes[1] >> 4);      
	    ...
	    //sResult = (bytes[0] << 8) + bytes[1];
	    //sResult = 0x3ff & (sResult >> 2);       // VB: This is when the converter is 10 bits (7997).
	    //Result = (float)(sResult) * 3 / 1024;
		//Result = (u08)(Result); 
		
	return(Result);
}

u08 MP_sensor_read()   //Called from user_code.c
//*******************************/
{
	u08 saddr, scmd, bytes[2];
	//static float Result;
